...
export async function getStaticProps({ params }) {
  ...
  const countryRes = await fetch(
    `https://api.farmsubsidy-next.medienrevolte.de/countries?country=${params.country}`
  );
  ...
  const recipientsRes = await fetch(
    `https://api.farmsubsidy-next.medienrevolte.de/recipients?country=${params.country}&order_by=-amount_sum&limit=5`
  );
