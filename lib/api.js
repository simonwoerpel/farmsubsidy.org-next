...
export async function getCountries(query = {}, includeMeta = false) {
  return await api("countries", query, includeMeta);
}

export async function getCountry(country, query = {}) {
  const res = await api("countries", { country, ...query });
  return res[0];
}

export async function getYears(query = {}) {
  return await api("years", query);
}

export async function getYear(year) {
  const res = await api("years", { year });
  return res[0];
}
...
export async function getSchemes(query = {}, includeMeta = true) {
  return await api(
    "schemes",
    { scheme_id__null: false, amount__null: false, ...query },
    includeMeta
  );
}

export async function getScheme(scheme_id) {
  const res = await api("schemes", { scheme_id });
  return res[0];
}
...
export async function getLocations(query = {}, includeMeta = true) {
  return await api(
    "locations",
    { recipient_address__null: false, amount__null: false, ...query },
    includeMeta
  );
}

export async function getLocation(recipient_address) {
  const res = await api("locations", { recipient_address });
  return res[0];
}
