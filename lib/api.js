...
export async function getAuthenticatedStatus() {
  const { status } = await api("authenticated", {}, true);
  return status;
}
...
export async function getNuts(level, query = {}) {
  const endpoint = `nuts${level}`;
  return await api(endpoint, Q.getNutsQuery(query));
}

export async function getAggregation(query = {}) {
  const res = await api("agg", {
    ...query,
    order_by: null,
    limit: null,
    p: null,
